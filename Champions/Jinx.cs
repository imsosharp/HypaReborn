...
namespace HypaJungle
{
    class Jinx : Jungler
    {
        ...
        public Jinx()
        {
            setUpSpells();
            setUpItems();
            LevelUpSeq = new int[] { 1,2,1,3,1,4,1,2,1,2,4,2,2,3,3,4,3,3};
            dragOnLvl = 7;
        }
        ...
        public override void UseQ(Obj_AI_Minion minion)
        {
            if (!Q.IsReady() )
                return;
            if (FishBoneActive && PowPowStacks < 3)
            {
                Q.Cast();
                return;
            }

            if (!FishBoneActive && PowPowStacks == 3 && JungleClearer.focusedCamp.aliveMinCount() > 1 && player.Mana>130)
            {
                Q.Cast();
            }
        }
        ...
        {
            if (W.IsReady() && camp.campPosition.Distance(player.Position, true) < 1200 * 1200)
            {
                W.Cast();
            }
        }
