...
namespace HypaJungle
{
    class JungleClearer
    {
        ...
        public static void updateJungleCleaner()
        {
            ...
            {
                if (jungler.recall.IsReady() && !player.IsChanneling && !ObjectManager.Player.InFountain() && !recalCasted)
                {
                    jungler.recall.Cast();
                    recalCasted = true;
                }
                ...
                if (ObjectManager.Player.InFountain())
                {
                    ...
                    {
                        if (ObjectManager.Player.InFountain() && player.Health > player.MaxHealth*0.7f &&
            ...
            if (jcState == JungleCleanState.GoingToShop)
            {
                ...
                if (ObjectManager.Player.InFountain() && player.IsChanneling)
                {
                ...
                {
                    if (ObjectManager.Player.InFountain() && player.Health > player.MaxHealth*0.8f &&
                        (!jungler.gotMana || player.Mana > player.MaxMana * 0.8f) && (jungler.nextItem == null || player.GoldCurrent+40 <= jungler.nextItem.goldReach))
                    {
                        jcState = JungleCleanState.SearchingBestCamp;
                        Console.WriteLine("SearchingBestCamp");
                    }
                    if (!player.IsChanneling && !ObjectManager.Player.InFountain())
                    {
                        jcState = JungleCleanState.SearchingBestCamp;
                        Console.WriteLine("SearchingBestCamp");
                    }
            ...
            if (jcState == JungleCleanState.GoingToShop && ObjectManager.Player.InFountain())
            {
